...
pub unsafe fn rtl_init_unicode_string(
    ...
    unsafe {
        ...
        let func: FnRtlInitUnicodeString =
            core::mem::transmute(table.win32.rtl_init_unicode_string);
        func(destinations_string, source_string);
    }
}

pub unsafe fn rtl_get_version(prtl_osversioninfow: *mut OsVersionInfoExW) -> NTSTATUS {
    unsafe {
        let table = ssn_table::syscall_table();
...
pub unsafe fn tp_port_work(work: *mut c_void) {
    unsafe {
        let table = ssn_table::syscall_table();
...
pub unsafe fn tp_release_work(work: *mut c_void) {
    unsafe {
        let table = ssn_table::syscall_table();
...
pub unsafe fn rtl_destroy_process_parameters(process_parameters: *mut c_void) {
    unsafe {
        let table = ssn_table::syscall_table();
